...
public:
    ...
    Node* searchPost(int search_id) {
        ...
        if (current_post == nullptr) {
            ...
            cout << "NAME: " << current_post->name << " TEXT: " << current_post->text << endl;
        }
        return current_post;
    }
    ...
    void saveToFile(const string& filename) {
        ...
        if (file.is_open()) {
            Node* current_post = head;
            while (current_post != nullptr) {
                file << current_post->id << ',' << current_post->time << ',' << current_post->name << ',' << current_post->text << '\n';
                current_post = current_post->next_post;
            }
            file.close();
    ...
    void loadFromFile(const string& filename) {
        ...
        if (file.is_open()) {
            ...
            while (file >> id >> ws && getline(file, time, ',') && getline(file, name, ',') && getline(file, text)) {
